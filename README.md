...
final_submission/
├── README.md        ← this file
├── attack_best.py   ← attack script — generates submission_best.zip
└── submission.py    ← leaderboard submission script (pre-configured)
```

> **`submission_best.zip` is generated by running `attack_best.py`** (Step 3 below).  
> Once generated, run `submission.py` to submit it to the leaderboard.
...
The script automatically finds the dataset at `../tml2026_task4/` and writes `submission_best.zip` directly into this folder.
